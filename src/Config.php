...
class Config
{
    ...
    public function merge($config)
    {
        ...
        if (!empty($config['extra']['whmyr/yamlenv']) && is_array($config['extra']['whmyr/yamlenv'])) {
            foreach ($config['extra']['whmyr/yamlenv'] as $key => $val) {
                $this->config[$key] = $val;
            }
        }
    }
        ...
        if ($config === null) {
            $baseDir = realpath(
                substr(
                    ...
                    0,
                    -strlen($composerConfig->get('vendor-dir', self::RELATIVE_PATHS))
                )
            );
